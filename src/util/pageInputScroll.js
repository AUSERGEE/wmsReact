/*creat by lxf 2018/08/29*/ 
/*解决input输入框在安卓手机下被手机软键盘遮挡问题*/ 

function pageInputScroll() {
    
    let initWindowHeight=window.innerHeight
    setTimeout(() => {
      let wrapDiv=document.getElementsByClassName('animated-router-forward-enter-done')[0]||document.getElementsByClassName('animated-router-backward-enter-done')[0]
    ...
    window.onresize=function(){ //如果浏览器窗口高度缩小25%以上，就认为是软键盘出来了
        if(initWindowHeight-window.innerHeight>initWindowHeight/4&&document.querySelectorAll(':focus').length>0){
           if(offset(document.querySelectorAll(':focus')[0]).top>initWindowHeight/4){
             ...
             console.log(offset(document.querySelectorAll(':focus')[0]).top-initWindowHeight/4,'d')
           }
